# How codeflow3d works

The long version. The [README](../README.md) is the short one — this is
everything behind it: how the call graph is resolved, what every element in the
scene means, how a save reaches the screen in 8ms, and how to get keystroke-level
updates out of an editor.
...
![The repo drawer, with a confidence bar per resolution tier](repo-drawer.png)
...
![One screen filled to the frame: tab strip, line numbers, indent guides, minimap, flow rails, status bar](screen-zoom.png)
...
![codeflow3d tracing its own source: the wall of screens, the call bundle, and the module tree](live-graph.png)
...
![A file open flat: line numbers, VS Code's palette, flow rails, and the line the screen was following](flat-editor.png)
...
![The changes drawer, with per-file diffs and graph statistics](change-log.png)
